...
class ProductsController extends Controller
{
    public function index()
    {
        ...
        $tshirts = Tshirt::with('images')
            ...
            ->through(function ($tshirt) {
                return [
                    ...
                    'otherImages' => $tshirt->getOtherImages() ?: collect([]), // Ensure it's always a collection
        ...
        $products = Product::with(['productType', 'designTemplate', 'images']) // Include images relationship for template products
            ->orderBy('created_at', 'desc')
            ->paginate(9)
            ->through(function ($product) {
                // For template-based products, get their images similar to tshirts
                $mainImage = $product->images->where('order', 1)->first();
                $otherImages = $product->images->filter(function ($image) {
                ...
                return [
                    ...
                    'mainImage' => $mainImage,
                    'otherImages' => $otherImages ?: collect([]), // Ensure otherImages is always defined
        ...
        return inertia('Admin/Products', [
            'products' => $paginator, // Return the paginator object directly
    ...
    public function edit($id)
    {
        ...
        $tshirt = Tshirt::with('images')->find($id);
        $product = Product::with(['productType', 'designTemplate'])->find($id);
        
        if ($tshirt) {
            // Edit legacy T-shirt
            return $this->editLegacyTshirt($tshirt);
        } elseif ($product) {
            // Edit template-based product
            return $this->editTemplateProduct($product);
        } else {
            abort(404, 'Product not found');
        }
    }
    
    private function editTemplateProduct(Product $product)
    {  // if (! Gate::allows('super_admin')) {
        //     return back()->withErrors([
        //         'authorization' => config('messages.guest_admin_restriction'),
        //     ]);
    ...
    private function editLegacyTshirt(Tshirt $tshirt)
    {  // if (! Gate::allows('super_admin')) {
        //     return back()->withErrors([
        //         'authorization' => config('messages.guest_admin_restriction'),
        //     ]);
    ...
    {  // if (! Gate::allows('super_admin')) {
        ...
        \Log::info('Store Product - is_template_based value:', [$request->input('is_template_based')]);
        \Log::info('Store Product - design_template_id value:', [$request->input('design_template_id')]);
        \Log::info('Store Product - isTemplateBased decision:', [
            'is_template_based_exists' => $request->has('is_template_based'),
            'is_template_based_raw' => $request->input('is_template_based'),
            'is_template_based_bool' => $request->boolean('is_template_based'),
            'is_template_based_strict_check' => $request->input('is_template_based') === true || $request->input('is_template_based') === 'true',
            'design_template_id_filled' => $request->filled('design_template_id'),
            'design_template_id_value' => $request->input('design_template_id'),
            'isTemplateBased_final' => ($request->has('is_template_based') && ($request->input('is_template_based') === true || $request->input('is_template_based') === 'true')) || 
                                  ($request->filled('design_template_id') && $request->input('design_template_id') !== '')
        ]);
    
        // Check if this is a template-based product creation
        // Use strict comparison to check for actual true values
        $isTemplateBased = ($request->has('is_template_based') && 
                       ($request->input('is_template_based') === true || 
                        $request->input('is_template_based') === 'true' ||
                        $request->input('is_template_based') == 1)) || 
                       ($request->filled('design_template_id') && 
                        $request->input('design_template_id') !== '' && 
                        $request->input('design_template_id') !== null);

        \Log::info('Store Product - Final decision: ' . ($isTemplateBased ? 'Template-based' : 'Legacy T-shirt'));

        if ($isTemplateBased) {
            // Handle template-based product creation
            \Log::info('Store Product - Processing as template-based product');
            return $this->storeTemplateProduct($request);
        } else {
            // Handle legacy T-shirt creation
            \Log::info('Store Product - Processing as legacy t-shirt');
    ...
    {
        // Log incoming request data for debugging
        \Log::info('Store Template Product Request Data:', $request->all());
    ...
    {
        // Log incoming request data for debugging
        \Log::info('Store Legacy Tshirt Request Data:', $request->all());
    ...
    {
        // Log incoming request data for debugging
        \Log::info('Update Product Request Data for ID '.$id.':', $request->all());
    ...
    {  // if (! Gate::allows('super_admin')) {
        ...
        // Log incoming request data for debugging
        \Log::info('Update Template Product Request Data:', $request->all());
    ...
    {  // if (! Gate::allows('super_admin')) {
        ...
        // Log incoming request data for debugging
        \Log::info('Update Legacy Tshirt Request Data:', $request->all());
