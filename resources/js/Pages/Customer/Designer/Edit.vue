    ...
    <div class="app-container">
      ...
      <aside class="tools-panel">
        <div class="tools-container">
          <div class="tool-item" v-for="tool in drawingTools" :key="tool.key">
            <button class="tool-icon" :class="{ active: activeTool === tool.key }" @click="activateTool(tool.key)">
              <i :class="tool.icon"></i>
            </button>
            <span class="tool-label">{{ t(`designer.${tool.key}_label`, tool.label) }}</span>
          </div>
          
          <div class="tool-item" v-for="tool in shapeTools" :key="tool.key">
            <button class="tool-icon" :class="{ active: activeTool === tool.key }" @click="activateTool(tool.key)">
              <i :class="tool.icon"></i>
            </button>
            <span class="tool-label">{{ t(`designer.${tool.key}_label`, tool.label) }}</span>
          ...
          <div class="tool-item" v-for="tool in mediaTools" :key="tool.key">
            <button class="tool-icon" :class="{ active: activeTool === tool.key }" @click="activateTool(tool.key)">
              <i :class="tool.icon"></i>
            </button>
            <span class="tool-label">{{ t(`designer.${tool.key}_label`, tool.label) }}</span>
          ...
          <div class="tool-item" v-for="tool in utilityTools" :key="tool.key">
            <button class="tool-icon" :class="{ active: activeTool === tool.key }" @click="activateTool(tool.key)">
              <i :class="tool.icon"></i>
            </button>
            <span class="tool-label">{{ t(`designer.${tool.key}_label`, tool.label) }}</span>
...
export default {
  ...
  methods: {
    ...
    loadExistingDesign() {
      // Load the design data from the prop
      const design = this.design
      if (!design) return

      // Set design name as draft text
      if (design.name) {
        this.draftText = design.name
      }

      // Apply template from the design if available
      ...
      // Load elements from design data
      if (design.design_data) {
        ...
        if (designData.activeTemplate) {
          this.applyTemplate(designData.activeTemplate, false)
        }
      }

      // Load elements from separate field if available
      if (design.elements && Array.isArray(design.elements)) {
        this.elements = design.elements
      }

      // Load drawing if available in design data
      if (design.design_data) {
        let designData;
        if (typeof design.design_data === 'string') {
          try {
            ...
            console.error('Error parsing design data for drawing:', e);
            designData = {};
          }
        } else {
          designData = design.design_data;
    ...
    async downloadDesign() {
      ...
      link.download = this.design?.name ? `${this.design.name}.png` : 'design.png'
      link.click()
    },
    async saveDesignToLibrary() {
      ...
      try {
        ...
        const payload = {
          id: this.design.id, // Use existing design ID for updates
          template_id: this.activeTemplate.id,
          name: this.design.name || `design-${new Date().toISOString()}`,
          preview: dataUrl,
          state: {
            activeTemplate: this.activeTemplate,
            elements: this.elements,
        ...
        // Update the existing design in the database
        await this.saveDesignToDatabase();
      } catch (e) {
    ...
    async saveDesignToDatabase() {
      ...
      try {
        ...
        if (this.$page.props.auth?.user?.id) {
          payload.user_id = this.$page.props.auth.user.id
        }
        ...
        if (!response.ok) {
          throw new Error(result.message || result.error || 'فشل التحديث في قاعدة البيانات')
        }

        alert(this.t('designer.updated_success', 'تم تحديث التصميم بنجاح!'))
        ...
        alert(this.t('designer.update_error', 'خطأ في تحديث التصميم: ') + error.message)
      } finally {
        this.pendingSave = false
      }
    },
